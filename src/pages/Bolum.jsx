            ...
            {Object.entries(kategoriler).map(([kategori, bolumler], i) => (
              <div key={i} className="p-4 rounded-xl bg-white ">
                ...
                {bolumler.map((dept, j) => (
                  ...
                  >
                    <h3 className="text-xl font-semibold group-hover:text-blue-700">{dept.name}</h3>
                    <p className="text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity duration-300 mt-2">
                      {dept.desc}
                    </p>
                  </div>
                ))}
